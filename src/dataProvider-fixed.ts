...
/**
 * Data Provider corregido para tu API
 * Usa los parámetros exactos que espera tu backend
 */
export const fixedDataProvider: DataProvider = {
  getList: async ({ resource, pagination, sorters, filters, meta }) => {
    console.log('=== FIXED DATA PROVIDER ===');
    ...
    // Paginación - usando los nombres exactos que espera tu API
    if (pagination) {
      params.currentPage = (pagination as any).current || 1;
      params.pageSize = pagination.pageSize || 10;
    }

    // Sorting - formato que espera tu API
    ...
    // Filters - formato que espera tu API
    ...
    try {
      ...
      console.error('Fixed DataProvider Error:', error);
